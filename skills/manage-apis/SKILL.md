...
An API artifact is a configuration record managed by the platform. Depending on its `apiType`, it exposes a server script over HTTP, gates and enriches proxy calls to an upstream service, or documents CRUD access to a table. `save_api` / `delete_api` write the same records the Cockpit's API Designer edits — **changes are live immediately, there is no draft/activate step**.
...
**`apiFormat`**: `0` = Swagger 2, `1` = OpenAPI 3. It selects the spec-generation branch and designer editors. **Always set `apiFormat: 1` for new APIs** (the Cockpit does; the default is 0 for legacy reasons, and the Swagger 2 branch depends on metadata MCP cannot write — see Definitions below).
...
|---|---|
| Key **absent** | Field untouched — existing value preserved. Partial updates are safe and preferred: `{ "id": "...", "enableTrace": true }` changes exactly one flag. |
...
Sending any of these does nothing — no error, no effect (all verified live or against the save schema):

- `roles` (plural) anywhere, and **`role` (singular) is accepted by the schema but ignored on save** — role assignment over MCP is a **silent no-op** (verified: save succeeded, `roles` stayed `[]`). Assign roles in the Cockpit only.
...
```
The endpoint MUST follow the formula (name lowercased, spaces/`/` stripped, prefixed `/api/serverscript/`). The runtime resolves `<name>` in the URL against the API's `endpoint` first, then **falls back to matching the artifact `name`** — so a wrong endpoint often still "works", hiding the misconfiguration until a rename or a duplicate name breaks it. `isPublic: true` + `/public/serverscript/<slug>` endpoint makes it callable without login at that public path.
...
- `paths[].operAccess: ["<roleId>", ...]` narrows individual operations *after* the API-level check passes (writable over MCP, but you can't discover role ids over MCP — `query_entity_table` only reads your own data tables; get ids from the Cockpit or an existing API's `roles`).
...
`enableTrace: true` (API-level) or `paths[].enableTrace` (per operation) records a trace entry per call: apiId, operationId, runtime (ms), `status` (`success`/`error`), calling application, and the request body only if `paths[].captureBody: true` (body capture = data exposure; ask before enabling). **The trace's `statusCode` is never populated** — judge outcomes by `status`. View traces in the Cockpit (API Trace); they are not readable over MCP. Noisy on high-volume APIs; also useful: `list_system_logs`/`get_system_log` (see inspect-system-logs skill) for script errors.
...
- Create/manage **API Authentication** artifacts, API groups' membership lists, or read role and trace data.
